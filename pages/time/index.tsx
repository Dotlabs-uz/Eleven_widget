	...
	async function getFreeTime() {
		try {
			...
			if (response.status === 200 || response.status === 201) {
				...
				const workDayStart = moment(selectDay).startOf("day").add(13, 'hours').toISOString()
				const workDayEnd = moment(selectDay).startOf("day").add(26, 'hours').toISOString();
				// console.log(moment(selectDay).add(10, "hours").toISOString(), "selected");
				console.log(workDayEnd, "selected");
